(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('stream'), require('encoding'), require('http'), require('url'), require('https'), require('zlib')) :
	typeof define === 'function' && define.amd ? define(['stream', 'encoding', 'http', 'url', 'https', 'zlib'], factory) :
	(factory(global.stream,global.encoding,global.http,global.url,global.https,global.zlib));
}(this, (function (stream,encoding,http,url,https,zlib) { 'use strict';
	...
	(function (global, factory) {
		...
		var realFetch = ( index_es && fetch ) || index_es;

		var fetch$1 = function (url$$1, options) {
		...
		fetch$1.fetch = fetch$1;
		fetch$1.Response = realFetch.Response;
		fetch$1.Headers = realFetch.Headers;
		fetch$1.Request = realFetch.Request;
		fetch$1.polyfill = false;

		var nodePonyfill = fetch$1;
		...
		class DefaultHttpClient {
		  ...
		  get(url$$1, params) {
		    ...
		    const _url = _params !== undefined
		      ? `${url$$1}?${this.formatQueryString(params)}`
		      : url$$1;

		    // eslint-disable-next-line no-console
		    console.log(`HttpService.get(${_url}`);

		    return nodePonyfill(_url)
		      .then(res => res.json())
		  }
		...
		class CatsService {
		  constructor(config, http$$1) {
		    this._config = config || {
		      url: 'http://localhost:9999',
		    };

		    this._http = http$$1 || new DefaultHttpClient();
		  }
	...
	var CatsService = unwrapExports(bundle);

	var Filter = urls => urls
	  .map(url$$1 => `<img src="${url$$1}" >`)
	  .join('\n')
